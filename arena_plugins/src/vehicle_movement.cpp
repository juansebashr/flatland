...
void VehicleMovement::OnInitialize(const YAML::Node& config) {
  ...
  pedsim_agents_sub_ = nh_.subscribe(pedsim_agents_topic, 1,
                                     &VehicleMovement::agentCallback, this);
...
void VehicleMovement::BeforePhysicsStep(const Timekeeper& timekeeper) {
  ...
  safety_dist_ =
      config["safety distance factor"][person.social_state].as<float>() *
      config["human obstacle safety distance radius"][person.type].as<float>();
  ...
  } else if (config["safety distance factor"][person.social_state].as<float>() <
             0.89) {
...
void VehicleMovement::set_safety_dist_footprint(b2Body* physics_body,
                                                double radius) {
...
void VehicleMovement::agentCallback(
...
PLUGINLIB_EXPORT_CLASS(flatland_plugins::VehicleMovement,
