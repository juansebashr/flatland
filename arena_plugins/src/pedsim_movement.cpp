 ...
 * @brief	 	The movement of the pedsim agents is as well applied to
 *the flatland models. Furthermore, a walking pattern is added.
...
void PedsimMovement::OnInitialize(const YAML::Node& config) {
  ...
  safety_dist_config = YAML::LoadFile(ros::package::getPath("simulator_setup") +
                                      "/safety_distance_parameter.yaml");
  ...
  std::normal_distribution<double> d_leg_offset{
      reader.Get<double>("leg_offset"), reader.Get<double>("var_leg_offset")};
  leg_offset_ = d_leg_offset(generator);

  std::normal_distribution<double> d_step_time{
      reader.Get<double>("step_time"), reader.Get<double>("var_step_time")};
  double step_time = d_step_time(generator);

  std::normal_distribution<double> d_leg_radius{
      reader.Get<double>("leg_radius"), reader.Get<double>("var_leg_radius")};
  ...
  std::string pedsim_agents_topic =
      ros::this_node::getNamespace() + reader.Get<std::string>("agent_topic");

  std::string agent_state_topic =
      reader.Get<std::string>("agent_state_pub", "agent_state");
  double update_rate = reader.Get<double>("update_rate");
  // update_timer_.SetRate(update_rate);  // timer to update global movement of
  // agent
  ...
  pedsim_agents_sub_ = nh_.subscribe(pedsim_agents_topic, 1,
                                     &PedsimMovement::agentCallback, this);
  // publish the socialPedsimMovement.Aft state of every pedestrain
  agent_state_pub_ =
      nh_.advertise<pedsim_msgs::AgentState>(agent_state_topic, 1);

  // Get bodies of pedestrian
  body_ = GetModel()
              ->GetBody(reader.Get<std::string>("base_body"))
              ->GetPhysicsBody();
  left_leg_body_ = GetModel()
                       ->GetBody(reader.Get<std::string>("left_leg_body"))
                       ->GetPhysicsBody();
  right_leg_body_ = GetModel()
                        ->GetBody(reader.Get<std::string>("right_leg_body"))
                        ->GetPhysicsBody();
  safety_dist_b2body_ =
      GetModel()
          ->GetBody(reader.Get<std::string>("safety_dist_body"))
          ->GetPhysicsBody();
  safety_dist_body_ =
      GetModel()->GetBody(reader.Get<std::string>("safety_dist_body"));
  safety_dist_b2body_danger_zone =
      GetModel()
          ->GetBody(reader.Get<std::string>("safety_dist_body"))
          ->GetPhysicsBody();
  safety_dist_body_danger_zone =
      GetModel()->GetBody(reader.Get<std::string>("safety_dist_body"));
  ...
  if (body_ == nullptr || left_leg_body_ == nullptr ||
      right_leg_body_ == nullptr || safety_dist_b2body_ == nullptr) {
    throw flatland_server::YAMLException(
        "Body with with the given name does not exist");
  }
...
void PedsimMovement::BeforePhysicsStep(const Timekeeper& timekeeper) {
  ...
  safety_dist_ =
      safety_dist_config["safety distance factor"][person.social_state]
          .as<float>() *
      safety_dist_config["human obstacle safety distance radius"][person.type]
          .as<float>();

  c = Color(0.26, 0.3, 0, safety_dist_body_alpha);
  if (safety_dist_config["safety distance factor"][person.social_state]
          .as<float>() > 1.2) {
    c = Color(0.93, 0.16, 0.16, safety_dist_body_alpha);
  } else if (safety_dist_config["safety distance factor"][person.social_state]
                 .as<float>() < 0.89) {
  ...
  if (useDangerZone == false) {
    ...
    if (vel > 0.01) {  // this threshold is used for filtering of some rare
                       // cases, no influence for performance
  ...
  if (init_ == true && agentCallbackReceived) {
    ...
    resetLegPosition(person.pose.position.x, person.pose.position.y,
                     angle_soll);
    init_ = false;
  }

  // Set pedsim_agent position in flatland simulator
  body_->SetTransform(b2Vec2(person.pose.position.x, person.pose.position.y),
                      angle_soll);
  safety_dist_b2body_->SetTransform(
      b2Vec2(person.pose.position.x, person.pose.position.y), angle_soll);
  safety_dist_b2body_danger_zone->SetTransform(
      b2Vec2(person.pose.position.x, person.pose.position.y), angle_soll);
  ...
  if (toggle_leg_movement_) {
    ...
    switch (state_) {
      // Right leg is moving
      case RIGHT:
        moveRightLeg(vel_x * vel_mult, vel_y * vel_mult,
                     (angle_soll - angle_ist));
        if (vel_mult == 0.0) {
          state_ = LEFT;
        }
        break;
      // Left leg is moving
      case LEFT:
        moveLeftLeg(vel_x * vel_mult, vel_y * vel_mult,
                    (angle_soll - angle_ist));
        if (vel_mult == 0.0) {
          state_ = RIGHT;
        }
...
float PedsimMovement::getValueInRange(float value_in, float value_max,
                                      float value_min) {
...
void PedsimMovement::moveLeftLeg(float32 vel_x, float32 vel_y,
                                 float32 angle_diff) {
  double max_vel = 1.8;
  double min_vel = -1.8;
...
void PedsimMovement::moveRightLeg(float32 vel_x, float32 vel_y,
                                  float32 angle_diff) {
  double max_vel = 1.8;
  double min_vel = -1.8;
  double v_x = getValueInRange(vel_x, legs_max_speed, -legs_max_speed);
...
// If the legs are both at the 0-point, they get corrected to the position from
// pedsim. This is necessary to regularily synchronize legs and pedsim agents.
...
void PedsimMovement::agentCallback(
...
// This is necessary to be able to set the leg radius auto-generated with
// variance original function just applies the defined radius in yaml-file.
// other option: modify flatland package, but third-party
void PedsimMovement::set_circular_footprint(b2Body* physics_body,
                                            double radius) {
...
// variance original function just applies the defined radius in yaml-file.
// other option: modify flatland package, but third-party
void PedsimMovement::set_safety_dist_footprint(b2Body* physics_body,
                                               double radius) {
  Vec2 center = Vec2(0, 0);
  b2FixtureDef fixture_def;
...
// Copied this function from model_body.cpp in flatland folder
// This is necessary to be able to set the leg radius auto-generated with
// variance original function just applies the defined properties from
// yaml-file. other option: modify flatland package, but third-party
...
void PedsimMovement::calculateDangerZone(float vel_agent) {
  ...
  // ROS_INFO("safty model pE0[%f]dangerZoneRadius[%f]dangerZoneAngle[%f]",
  // pL[0], dangerZoneRadius, dangerZoneAngle);
...
PLUGINLIB_EXPORT_CLASS(flatland_plugins::PedsimMovement,
